(function (factory) {
    ...
    return function koreBotChat() {
        ...
        function updateOnlineStatus() {
            ...
            chatWindow.prototype.renderMessage = function (msgData) {
                ...
                if (messageHtml === '' && msgData && msgData.message && msgData.message[0]) {
                    ...
                    // custom template for group button
                    else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "group_button") {
                        messageHtml = $(me.getChatTemplate("templategroupbutton")).tmpl({
                            'msgData': msgData,
                            'helpers': helpers,
                            'extension': extension
                        });
            ...
            var chatWindowTemplate = '<script id="chat_window_tmpl" type="text/x-jqury-tmpl"> \
                ...
                <div class="kr-wiz-menu-chat defaultTheme-kore">\
                    ...
                    <span class="closeChatBodyModal d-none"></span>\
            ...
            // custom template for group button
            var groupButtonTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
                {{if msgData.message}} \
                    <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
                        class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon"> \
                        <div class="buttonTmplContent"> \
                            ...
                            <ul class="buttonTmplContentBox">\
                                ...
                                {{each(key, msgItem) msgData.message[0].component.payload.buttons}} \
                                    <a href=""#>\
                                        <li class="buttonTmplContentChild {{if key > 3}} d-none hidden{{/if}}" {{if msgData}}msgData="${JSON.stringify(msgData)}"{{/if}} {{if msgItem.payload}}value="${msgItem.payload}"{{/if}} {{if msgItem.payload}}actual-value="${msgItem.payload}"{{/if}} {{if msgItem.url}}url="${msgItem.url}"{{/if}} data-value="${msgItem.value}" type="${msgItem.type}">\
                                            ${msgItem.title}\
                                        </li> \
                                    </a> \
                                {{/each}} \
                                <div>\
                                    <li class="buttonTmplContentChild hidden" id="showMore" style="color: #404040">\
                                        もっと見る\
                                    </li> \
                                </div> \
                            </ul>\
     ...
     var iframe = '<script id="chat_message_tmpl" type="text/x-jquery-tmpl"> \
            ...
            } else if (tempType === "templategroupbutton") {    // custom template for group button
                return groupButtonTemplate;
            ...
            function showMoreBtn() {
                var hiddenBtns = document.getElementsByClassName('hidden');
                var showMore = document.getElementById('showMore');
                showMore.onclick = function () {
                    [].forEach.call(hiddenBtns, function (item) {
